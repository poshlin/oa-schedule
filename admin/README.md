...
跟 team.html 一樣：Sheet **檔案 → 共用 → 發布到網路** → 分頁選「今日待辦」、格式「逗號分隔值 (.csv)」→ 發布 → 複製 URL → 填進 `admin.html` 與 `admin-review.html` 的 `TODO_CSV_URL`。

（「今日待辦」第一次要等掃描器 publish 過才會出現；可以先跑 `python3 scan.py --limit 3 --publish` 產生分頁再發布。）
...
然後重跑 `sh ~/Documents/OA_業務行政減負_2026-09-20/scanner/setup_macmini.sh`。
...
4. Mac mini：`python3 executor.py`（預設 dry-run）→ 印出「我會做什麼」→ 你看過沒問題 → `python3 executor.py --apply` → Corp 小紅點多一行、判斷區 status 變 `done`、evidence 有版本歷程時間
